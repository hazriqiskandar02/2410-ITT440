...
void update_database(const char *username) {
    ...
    if (mysql_query(conn, query)) {
        ...
        printf("Database updated successfully for user 2: %s\n", username);
    }
...
int main() {
    ...
    server_addr.sin_port = htons(1027);  // Unique port for server 2
    server_addr.sin_addr.s_addr = INADDR_ANY;

    bind(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    printf("C Server 2 running on port 1027...\n");

    while (1) {
        // Periodic updates for "Atan_Comel2"
        time_t current_time = time(NULL);
        if (current_time - last_update_time >= 30) {  // Every 3 seconds
            update_database("Atan_Comel2");
        ...
        if (recv_len > 0) {
            ...
            if (strncmp(buffer, "GET_POINTS:", 11) == 0) {
                ...
                char *username = buffer + 11;  // Extract username
                get_points(username, response);
            } else if (strncmp(buffer, "UPDATE:", 7) == 0) {
                // Handle UPDATE request
                char *username = buffer + 7;  // Extract username
